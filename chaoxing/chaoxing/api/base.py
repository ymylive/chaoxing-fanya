...
from .cookies import save_cookies, use_cookies
from .decode import (
    decode_course_list,
    decode_course_point,
    decode_course_card,
...
from .process import show_progress
...
class Chaoxing:
    ...
    def __init__(self, account: Account = None, tiku: Tiku = None,**kwargs):
        ...
        self.des_cipher = DESCipher()  # 新增DES加密器
        self.tiku = tiku
        self.kwargs = kwargs
        self.rollback_times = 0
        # 可选代理支持（GUI 可传入 http_proxy）
        self.http_proxy = (kwargs or {}).get("http_proxy")
        if self.http_proxy:
            # 设置环境代理，确保后续 init_session 也能使用
    ...
    def login(self):
        ...
        _session.verify = False
        # 应用代理（如有）
        if getattr(self, "http_proxy", None):
            _session.proxies = {"http": self.http_proxy, "https": self.http_proxy}
        logger.trace(f"login(): using proxy={getattr(self, 'http_proxy', None)}")
        logger.trace("login(): begin")
        
        # 首先尝试新的API接口（失败不提前返回，始终回退旧版）
        logger.trace("正在尝试新版登录API...")
        try:
            # 使用新的DES加密方式
            encrypted_password = self.des_cipher.encrypt(self.account.password)
            
            # 新的登录API使用GET请求
            _url = "https://passport2-api.chaoxing.com/v11/loginregister"
            _params = {
                "code": encrypted_password,
            ...
            logger.trace(f"login(new): url={_url}, headers={list(gc.HEADERS.keys())}, params={{'code':'***','uname': '***', 'loginType':'1', 'roleSelect':'true'}}")
            resp = _session.get(_url, headers=gc.HEADERS, params=_params)
            
            if resp and resp.status_code == 200:
                try:
                    resp_json = resp.json()
                    logger.trace(f"login(new): status={resp.status_code}, json={str(resp_json)[:500]}")
                    # 检查响应格式
                    if resp_json.get("result") == 1 or resp_json.get("status") == True:
                        save_cookies(_session)
                        logger.info("新版API登录成功...")
                        logger.trace(f"login(new): cookies_keys={list(_session.cookies.get_dict().keys())}")
                        return {"status": True, "msg": "登录成功"}
                    else:
                        # 仅记录，不提前返回，继续走旧版接口
                        error_msg = resp_json.get("msg", resp_json.get("errorMsg", resp_json.get("mes", "未知错误")))
                        logger.warning(f"login(new): 失败，将回退旧版: {error_msg}")
                except Exception as json_e:
                    logger.warning(f"login(new): 响应解析失败: {json_e}")
                    logger.trace(f"login(new): raw={resp.text[:500]}...")
                    
        except Exception as e:
            logger.warning(f"新版API登录失败，尝试回退到旧版: {e}")
        
        # 回退到旧版API
        logger.trace("正在尝试旧版登录API...")
        ...
        logger.trace(f"login(old): url={_url}, headers={list(gc.HEADERS.keys())}, data_keys={list(_data.keys())}")
        resp = _session.post(_url, headers=gc.HEADERS, data=_data)
        if resp and resp.status_code == 200:
            try:
                resp_json = resp.json()
                logger.trace(f"login(old): status={resp.status_code}, json={str(resp_json)[:500]}")
                # 修复：旧版API成功时status可能是True(布尔值)，也可能只是包含url字段
                if (resp_json.get("status") == True or 
                    resp_json.get("status") is True or 
                    ("url" in resp_json and resp_json.get("status") != False)):
                    save_cookies(_session)
                    logger.info("旧版API登录成功...")
                    logger.trace(f"login(old): cookies_keys={list(_session.cookies.get_dict().keys())}")
                    return {"status": True, "msg": "登录成功"}
                else:
                    error_msg = resp_json.get("msg2", resp_json.get("msg", resp_json.get("errorMsg", "登录失败")))
                    return {"status": False, "msg": str(error_msg)}
            except Exception as e:
                logger.error(f"login(old): 响应解析失败: {e}")
                logger.trace(f"login(old): raw={resp.text[:500]}...")
                return {"status": False, "msg": "登录响应解析失败"}
        else:
            logger.error(f"login(old): 请求失败，状态码: {resp.status_code if resp else 'None'}")
            return {"status": False, "msg": f"登录请求失败 (状态码: {resp.status_code if resp else 'None'})"}
    ...
    def get_fid(self):
        ...
        logger.trace("courses: init session")
    ...
    def get_course_list(self, force_old_api=False):
        _session = init_session()
        
        # 检查是否强制使用旧版API
        if not force_old_api:
            # 首先尝试新的课程列表API
            logger.trace("正在尝试新版课程列表API...")
            try:
                _new_url = "http://mooc1-api.chaoxing.com/mycourse/backclazzdata"
                _new_params = {"view": "json", "rss": "1"}
                _new_headers = {
                    "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/129.0.0.0 Safari/537.36 Edg/129.0.0.0",
                    "Accept": "application/json, text/javascript, */*; q=0.01",
                    "Referer": "http://mooc1-api.chaoxing.com/",
                }
                
                logger.trace(f"courses(new): url={_new_url}, headers={list(_new_headers.keys())}, params={_new_params}")
                _new_resp = _session.get(_new_url, headers=_new_headers, params=_new_params)
                
                if _new_resp and _new_resp.status_code == 200:
                    logger.info("新版课程列表API读取完毕...")
                    logger.trace(f"courses(new): status={_new_resp.status_code}, text_head={_new_resp.text[:500]}...")
                    # 解析新版API的JSON响应
                    try:
                        json_data = _new_resp.json()
                        if isinstance(json_data, list) and json_data:
                            # 转换为统一格式
                            course_list = []
                            for course in json_data:
                                # 调试日志：记录原始课程数据
                                logger.trace(f"新版API原始课程数据: {course}")
                                
                                course_detail = {
                                    "id": course.get("id", ""),
                                    "courseId": course.get("courseId", course.get("courseid", "")),
                                    "clazzId": course.get("clazzId", course.get("clazzid", "")),
                                    "cpi": course.get("cpi", ""),
                                    "title": course.get("title", course.get("coursename", "")),
                                    "desc": course.get("desc", ""),
                                    "teacher": course.get("teacher", course.get("teachername", "")),
                                    "roleid": course.get("roleid", ""),
                                    "info": course.get("info", "")
                                }
                                
                                # 调试日志：记录转换后的课程数据
                                logger.trace(f"新版API转换后课程数据: {course_detail}")
                                
                                # 验证关键字段
                                missing_fields = []
                                if not course_detail.get('courseId'):
                                    missing_fields.append('courseId')
                                if not course_detail.get('clazzId'):
                                    missing_fields.append('clazzId')
                                if not course_detail.get('cpi'):
                                    missing_fields.append('cpi')
                                    
                                if missing_fields:
                                    logger.warning(f"课程 '{course_detail.get('title')}' 缺失关键字段: {missing_fields}")
                                
                                course_list.append(course_detail)
                            if course_list:
                                # 检查是否有足够的有效课程（关键字段完整）
                                valid_courses = [c for c in course_list if c.get('courseId') and c.get('clazzId') and c.get('cpi')]
                                if len(valid_courses) < len(course_list) * 0.8:  # 如果80%以上课程数据不完整
                                    logger.warning(f"新版API数据质量差: {len(valid_courses)}/{len(course_list)} 课程数据完整，回退到旧版API")
                                else:
                                    return course_list
                    except Exception as e:
                        logger.warning(f"新版API响应解析失败: {e}")
                        
            except Exception as e:
                logger.warning(f"新版课程列表API失败，尝试回退到旧版: {e}")
        else:
            logger.info("强制使用旧版课程列表API...")
        
        # 回退到旧版课程列表API
        logger.trace("正在尝试旧版课程列表API...")
        ...
        logger.trace(f"courses(old): url={_url}, headers={list(_headers.keys())}, data={_data}")
        _resp = _session.post(_url, headers=_headers, data=_data)
        # logger.trace(f"原始课程列表内容:\n{_resp.text}")
        logger.info("旧版课程列表读取完毕...")
        logger.trace(f"courses(old): status={_resp.status_code}, text_head={_resp.text[:500]}...")
